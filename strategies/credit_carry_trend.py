...
Carry leg  = CREDIT / default-risk premium. Long HYG (high-yield corporate ETF) financed by a SHORT,
             ...
             the documented "credit-spread puzzle"). This is the PRO-CYCLICAL leg: it earns the spread in
             calm regimes and crashes when spreads blow out in stress. Total-return (dividend-ADJUSTED)
             series are used because the COUPON IS THE CARRY — un-adjusted Close would zero the premium.
Trend leg  = the FROZEN validated Boreas 21-market cross-asset TSMOM (crisis-alpha "smile" that pays big
             moves / bleeds chop). Vol-matched 50/50 against the credit leg.

THE THESIS (pre-registered, frozen). Credit-carry and trend are COMPLEMENTARY opposites: credit earns
the calm and crashes exactly when spreads gap out (GFC-2008, COVID-2020, 2022); trend is crisis-alpha
that pays those same tails. With corr(credit,trend) ~ 0 the trend leg acts as a near-mechanical hedge
that cuts the credit drawdown (the analogue of the crypto-carry 26.5%->14.5% DD-cut result) at little
Sharpe cost. The DIVERSIFICATION is the edge, not either standalone leg. PRIMARY pre-registered metric =
the combined book's MAR / max-drawdown and leg correlation on the write-once 2022+ holdout (the 2022
spread-widening is the key out-of-sample hedge test), via the standard CPCV/DSR/PBO/FDR rails.

WHY-NOT-DUPLICATE. The credit/default premium is absent from prior tests: crypto funding-carry (Midas
near-miss), DM FX+bond TERM carry (CLOSED), commodity term-structure carry (FAIL). Credit-spread carry
is compensation for systematic default/illiquidity risk — structurally distinct from bond roll/term
carry and from cross-sectional equity factors. Pairs with the validated trend hedge; also a pro-cyclical
candidate for the multi-premium book if the crypto-carry forward verdict (2026-08-28) fails.
...
total-return Close via yfinance directly (same direct-yfinance pattern the VRP book uses). HYG inception
Apr-2007 covers the full GFC; IEI/IEF/LQD reach 2007. The frozen 0.78 HY/IEI duration hedge ratio is
locked from ETF fact-sheet effective durations. Both legs' signals are lagged 1 day (no look-ahead);
8bps frozen cost on turnover (ETF spread+commission is far lower — deliberately conservative). The
...
def _monthly_trades(pos: pd.Series, credit_tk, treas_tk, h, r_credit, r_treas, lo) -> list:
    """One trade per monthly held run per leg (long credit / short duration-matched Treasury), within
    the overlap window — for deployment-sanity breadth in the 'Credit' sector."""
    out, p = [], pos[pos.index >= lo]
    legs = {credit_tk: (+1.0, 1.0, r_credit), treas_tk: (-1.0, h, r_treas)}
...
def signal(panel, credit_tk="HYG", treas_tk="IEI", hedge_ratio=0.78, credit_vol=0.10,
    ...
    pos = inv_vol_position(sig, cr, target_vol=credit_vol, vol_lb=vol_lb,
                           max_pos=max_pos, rebalance=rebalance)["CREDIT"]
    ...
    combo = blend * _vol_scale(df["credit"]) + (1 - blend) * _vol_scale(df["trend"])
